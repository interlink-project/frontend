    ...
    <DialogContent sx={{ minHeight: "60vh", backgroundColor: "background.default" }}>
      ...
      {(team && !loading) ? <Grid container>
        ...
        < Grid item md={8} sx={{ p: 2 }}>
          ...
          {tabValue === "members" && <UsersList size="small" users={team.users} searchOnOrganization={isAdmin && team.organization_id} disableHeader={false} onSearchResultClick={isAdmin && addUserToTeam} getActions={(user) => isAdmin && (
            [
              {
