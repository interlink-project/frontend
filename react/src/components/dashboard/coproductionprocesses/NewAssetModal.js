    ...
    >
            ...
            {can_open_in_modal
              ? (
                <>
                  ...
                  <iframe
                    style={{ display: loadingInstantiator ? 'none' : 'block' }}
                    src={`${selectedInterlinker.backend}/instantiate`}
      ...
      <DialogActions sx={{ bgcolor: 'background.default', justifyContent: 'center' }}>
        ...
        <Button
          ...
          variant='outlined'
          color='warning'
          onClick={() => window.open(`${selectedInterlinker.link}/preview`, '_blank')}
        >
          {selectedInterlinker.softwareinterlinker.preview_text}
          {' '}
          {t('(it will be not related to project but it is useful for features exploration)')}
        </Button>
        )}
        {downloadable && (
        <Button
          ...
          variant='outlined'
          color='warning'
          onClick={() => window.open(`${selectedInterlinker.link}/download`, '_blank')}
        >
        ...
        {instantiatable && (
        <LoadingButton
          loading={loadingKnowledgeInstantiation}
        ...
        <Button
          startIcon={<DoubleArrow />}
          sx={{ my: 2, mx: 4 }}
          disabled={!selectedInterlinker.uri}
          autoFocus
          variant='contained'
          onClick={() => window.open(selectedInterlinker.uri, '_blank')}
        >
          {t('Open the external resource on a new tab')}
        </Button>
        )}
      </DialogActions>
      )}
