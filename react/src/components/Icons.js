...
export const InProgressIcon = () => (
  <Loop
    fontSize='small'
    sx={{ color: statusColor('in_progress') }}
  />
);
...
export const StatusText = ({ status, t }) => <>{status === 'finished' ? t('Finished') : status === 'in_progress' ? t('In progress') : t('Awaiting')}</>;
